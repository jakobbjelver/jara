...
Renders the JARA app icon (a neon-yellow broadcast/megaphone mark: three
echo chevrons aimed at a rounded square) and the mark alone (for splash) to
every required platform asset. The authoritative artwork is the SVGs in
brand/source/ (jara-icon-square.svg, jara-icon-rounded.svg, jara-emblem.svg),
rasterized once to 1536px PNGs by tool/raster_brand_sources.sh (cairosvg,
alpha-preserving). This script only scales those canonical rasters — it never
redraws the mark.
...
BRAND_BG = (10, 10, 12)           # #0A0A0C — splash / launch / dark field
BRAND_BG_LIGHT = (21, 23, 26)     # #15171A — dark gradient light end
BRAND_NEON = (255, 230, 0)        # #FFE600 — neon yellow (the mark)
BRAND_NEON_HI = (255, 248, 107)   # #FFF86B — neon highlight
...
def _rgb(size: tuple[int, int]) -> Image.Image:
    """A flat brand-dark splash surface."""
    img = Image.new("RGBA", size, (*BRAND_BG, 255))
    return img
...
def generate_all() -> list[Path]:
    ...
    for filename, (w, h) in IOS_SPLASHES:
        ...
        canvas = Image.new("RGBA", (w, h), (*BRAND_BG, 255))
